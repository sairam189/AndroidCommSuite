...
def a2dp_connect(android_device):

    for a in android_device:
        droid, ed = a.droid, a.ed
        log.info("Coonect to remote device using a2dp: {}".format(
            droid.getBuildSerial()))
        time.sleep(10)
        if not a.droid.bluetoothA2dpConnect("PLT_Legend"):
            return False
    return True
...
def pair_dev_to_headset(log, pri_droid, dev_to_pair):

    time.sleep(10)
    log.info("Bonding device {} to {}".format(pri_droid.droid.bluetoothGetLocalAddress(),
                                              dev_to_pair))
    pri_droid.droid.bluetoothStartPairingHelper()
    result = pri_droid.droid.bluetoothDiscoverAndBond(dev_to_pair)
    end_time = time.time() + DEFAULT_TIMEOUT
    log.info("Verifying devices are bonded")
    time.sleep(5)
    while time.time() < end_time:
        bonded_devices = pri_droid.droid.bluetoothGetBondedDevices()
        bonded = False
        for d in bonded_devices:
...
def connect_dev_to_headset(log, pri_droid, dev_to_connect, profiles_set):
    ...
    paired = False
    for paired_device in pri_droid.bluetoothGetBondedDevices():
        if paired_device['address'] == \
                dev_to_connect:
            paired = True
            break
    ...
    if not paired:
        log.info("{} not paired to {}".format(pri_droid.getBuildSerial()))
        return False

    pri_droid.bluetoothConnectBonded(dev_to_connect)
    return True

def play_media(log, pri_droid, path):

    pri_droid.mediaPlayOpen("file:///sdcard/Music/{}".format(
                path))
    pri_droid.mediaPlaySetLooping(True)
    time.sleep(5)
    
    log.info("Music Streaming check done...")
    time.sleep(30)
    log.info("Music is now playing on device {}".format(
                pri_droid.serial))
    return True
